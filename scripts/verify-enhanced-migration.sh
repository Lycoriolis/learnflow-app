...
if curl -s --connect-timeout 5 http://localhost:5174/ > /dev/null; then
    ...
    SERVER_STATUS="✅"
elif curl -s --connect-timeout 5 http://localhost:5173/ > /dev/null; then
...
# Check course page migration
if grep -q "EnhancedMarkdownRenderer" src/routes/courses/\[...slug\]/+page.svelte; then
    echo "✅ Course page migrated to EnhancedMarkdownRenderer"
    COURSE_MIGRATED="✅"
else
    echo "❌ Course page not migrated"
    COURSE_MIGRATED="❌"
fi
...
if curl -s "http://localhost:5173/enhanced-test" | grep -q "Enhanced Markdown Components Test"; then
...
katex_count=$(curl -s "http://localhost:5173/enhanced-test" | grep -o "katex" | wc -l)
...
# Test notepad page
if curl -s "http://localhost:5173/tools/notepad" | grep -q "Notepad"; then
